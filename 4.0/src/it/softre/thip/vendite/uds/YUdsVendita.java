...
/**
 * 
 * @author DSSOF3 DSSOF3 70687 04/10/2022 Prima stesura.
 */
...
public class YUdsVendita extends YUdsVenditaPO {
	...
	protected static final String STMT_UDS_PADRE_CLG = "SELECT * "
			...
			+ "AND U."+YUdsVenditaTM.R_ID_UDS_PADRE+" = ? ";

	public static CachedStatement cUdsPadreCollegate = new CachedStatement(STMT_UDS_PADRE_CLG);
	...
	protected static final String STMT_ESIST_UDS_FIGLIE_CON_RIGHE = "SELECT * FROM SOFTRE.YUDS_VEN_RIG R  "
			...
			+ "AND R.ID_UDS IS NOT NULL";

	public static CachedStatement cEsistonoUdsFiglieConRows = new CachedStatement(STMT_ESIST_UDS_FIGLIE_CON_RIGHE);

	protected boolean iCopia;

	protected boolean assegnaNumeratoreAutomatico = true;

	protected boolean isRicalcoloPesi = false;
	...
	protected BigDecimal[] getPesiTotali() throws SQLException {
		...
		while (iterRighe.hasNext()) {
			YUdsVenRig udsVenRig = (YUdsVenRig) iterRighe.next();
			pesoLordo = pesoLordo.add(udsVenRig.getPesoLordo());
			pesoNetto = pesoNetto.add(udsVenRig.getPesoNetto());
		}
		if(tipoUds.getPeso() != null) {
			pesoLordo = pesoLordo.add(tipoUds.getPeso());
			pesoUds = pesoUds.add(tipoUds.getPeso());
		}
